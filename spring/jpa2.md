...
***정리***
- 영속성 컨텍스트는 JPA가 실제 DB와 애플리케이션에 존재하는 영역으로 DB 관리를 용이하기 위한 영역이다.
- CRUD에 대한 작업이 JPA에서 어떻게 동작하는지 이해하기 위해선 영속성 컨텍스트에 대한 개념을 알고 사용해야한다.
- 플러시는 영속성 컨텍스트를 비우는게 아닌 영속성 컨텍스트의 변경내용을 데이터베이스와 동기화하는 작업이다.
